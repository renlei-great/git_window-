...
# 删除a股中,day5_realtime_bars某一条信息
def delete_one_a_date_price(conn, assets_id, table_name):
    sql = "delete from %s where assets_id=%s" % (assets_id)
...
def jk_a_query_stock_quotation(conn, local_csv_name, end_date, exce_type='update'):
    with open(RESOURCE_PATH + '\\' + local_csv_name, 'r', encoding='utf8') as csvfile:
        ...
        for row in csv_reader:
            ...
            # 获取此股票在数据库中的信息
            select_sql = "select * from day5_realtime_bars where assets_id='%s'  order by datetime" % ('002668.XSHE')
            cursor = conn.cursor()
            ...
            # 判断是插入数据还是更新数据
            if len(real_all) == 5:  # 更像数据
                try:
                    # 获取单只股票的信息，并得到可用的返回值
                    temporary_list = jk_get_a_organize_data(assets_id, assets_name, end_date, 1)
                ...
                if len(temporary_list) > 0:
                    for price_dict in temporary_list:
                        delete_one_a_date_price(conn, real_all[0][0], 'day5_realtime_bars')
                        update_a_last_all_price(conn, price_dict, 'day5_realtime_bars')
            elif len(real_all) == 0:  # 插入数据
                # 如果列表不为空插入数据到数据库中
                if len(temporary_list) > 0:
                    for price_dict in temporary_list:
...
def a_day5_all_price_main(exce_type='update'):
    # 组织查询时间
