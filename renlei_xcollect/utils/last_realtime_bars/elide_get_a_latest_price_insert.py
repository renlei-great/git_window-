...
def a_last_all_price_func():
    ...
    # 存放所有未找到的A股
    not_astocks_list = []
    with open(RESOURCE_PATH + '\\xtech_astocks_sum.csv', 'r', encoding='utf8') as csvfile:
...
def insert_a_last_all_price(price_dict):
    ...
    cursor = conn.cursor()
    sql = "insert into realtime_bars_test (datetime, assets_id, assets_name, sell_id, " \
...
# 保存成csv文件
def save_csv_write_file(path_name, header, rows):
